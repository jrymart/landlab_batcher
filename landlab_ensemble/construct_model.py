...
def _expand_key_into_dict(key, value, current_dict):
    ...
    if len(split_key)==1:
        ...
        current_dict[split_key[0]] = {}
        return _expand_key_into_dict(split_key[1], value, current_dict[split_key[0]])
...
class ModelDispatcher:
    ...
    def __init__(self, database, model_class, filter=None):
    ...
    def dispatch_model(self):
        ...
        cursor = connection.cursor()
        run_id, model_parameters = cursor.execute("SELECT run_param_id, * FROM model_run_params WHERE %s" % self.filter_statement).fetchone()
        ...
        end_time = time.time()
        cursor = connection.cursor()
        metadata_update_statement = "UPDATE model_run_metadata SET model_end_time = %f WHERE model_rin_id = %s" %(end_time, model.run_id)
